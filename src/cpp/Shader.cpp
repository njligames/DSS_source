...
//              break;                                                           \
//            default:                                                           \
//              SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,  "Unknown (%x)", error);\
//            }                                                                  \
//        }                                                                      \
//    }                                                                          \
//  while (0);
...
#if !defined(NDEBUG)
    if (location == -1) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "The named attribute variable %s is not an active attribute in the specified program object or if name starts with the reserved prefix \"gl_\"", attributeName.c_str());
    }
#endif
    return location;
}
...
#if !defined(NDEBUG)
    if (location == -1) {
        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "The named attribute variable %s is not an active attribute in the specified program object or if name starts with the reserved prefix \"gl_\"", uniformName.c_str());
    }
#endif
    return location;
}
...
//            value.setFromOpenGLMatrix(m_mat4Buffer);
    ...
    bool Shader::getUniformValue(const char *uniformName,
                                        glm::vec3 &value)
    {
        int location = getUniformLocation(uniformName);
        if (location != -1)
...
//            std::cout << "Shader compile log:" << std::endl << log;
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,"Shader compile log: %s", log);
            free(log);
        }
#endif
...
bool Shader::validateProgram(GLuint programPointer)
{
    ...
    if (status == GL_FALSE)
    {
        ...
        if (logLength > 0)
        {
            ...
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Program validate log:\n%s", log);
            free(log);
        }
        return false;
    }
...
//    if (!_source)
//    {
//        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to load vertex shader");
//        return false;
//    }
//
//    shader = glCreateShader(type);
...
//            glGetShaderInfoLog(shader, logLength, &logLength, log);
//
//            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Shader compile log:\n%s", log);
//            free(log);
//        }
//
//        return false;
...
//            glGetProgramInfoLog(programPointer, logLength, &logLength, log);
//
//            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Program link log:\n%s", log);
//            free(log);
//        }
//
//        return false;
...
//    {
//        SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Failed to link program: %d", programPointer);
//
//        if (vertexShader) {
