...
mDetailDescription(game.getContent().getEditorial().getRecap().getMlb().getBlurb()),
mDetailImageTitle(game.getContent().getEditorial().getRecap().getMlb().getMedia().getHeadline().c_str()),
...
mReceivedListItemImageData(false) {
    ...
    for(std::vector<MLBJson::Cut>::const_iterator cut_iterator = _media.getImage().getCuts().begin();
    ...
    MLBJson::Photo _photo = game.getContent().getEditorial().getRecap().getMlb().getPhoto();
    
    largestSize = 0;
    for (std::map<std::string, MLBJson::Cut>::const_iterator cuts_iterator = _photo.getCuts().begin(); cuts_iterator != _photo.getCuts().end(); ++cuts_iterator) {
...
const std::string &GameModelViewData::getHomeName()const {
    return mHomeName;
}

const std::string &GameModelViewData::getAwayName()const {
    return mAwayName;
}
...
void *GameModelViewData::getDetailImageData(int *width, int *height, int *channels_in_file, size_t *fileSize, bool downloadImage) {
    ...
    if(nullptr == mDetailImageData) {
        ...
        mDetailImageData = (void *)stbi_load(base_path.c_str(), width, height, channels_in_file, 0);
...
void *GameModelViewData::getListItemImageData(int *width, int *height, int *channels_in_file, size_t *fileSize, bool downloadImage) {
    ...
    if(nullptr == mListItemImageData) {
        ...
        mListItemImageData = (void *)stbi_load(base_path.c_str(), width, height, channels_in_file, 0);
...
static void multi_image_download(CURLM *multi_handle, const std::vector<CURL*> &handleList, std::vector<FileData*> &fileDataVector) {
    ...
    while((msg = curl_multi_info_read(multi_handle, &msgs_left))) {
        if(msg->msg == CURLMSG_DONE) {
            ...
            SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "Transfer completed with status %d\n", msg->data.result);
        }
    }
...
void GameModelViewData::loadGames(const std::vector<MLBJson::Game> &games, std::vector<GameModelViewData*> &gvdVector) {
    ...
    for(std::vector<MLBJson::Game>::const_iterator iter = games.begin();
        ...
        fd = new FileData(MAX_IMAGE_WIDTH, MAX_IMAGE_HEIGHT, MAX_IMAGE_COMPONENTS);
    ...
    ThreadPool::getInstance()->enqueue(multi_image_download, multi_handle, handleList, fileDataVector);
}
...
void GameModelViewData::update(Publisher* who, void* userdata) {
    ...
    if(nullptr != completed_me &&
       nullptr != fd) {
        SDL_LogVerbose(SDL_LOG_CATEGORY_TEST, "This image is now loaded. (%s)\n", fd->_url.c_str());
    }
}
