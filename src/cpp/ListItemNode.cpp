...
mNextPosition(glm::vec3(0,0,0)),
mPrevPosition(glm::vec3(0,0,0)),
...
void ListItemNode::setNextPosition(const glm::vec3 &next) {mNextPosition=next;}

void ListItemNode::setPreviousPosition(const glm::vec3 &previousPosition) {mPrevPosition=previousPosition;}
...
void ListItemNode::scrollNext(float seconds, int algorithm){
    if(glm::length(mDirection) <= 0) {
        ...
        mTotalTime = std::max(seconds, std::numeric_limits<float>::denorm_min()) * 17;
        mCurrentTime = 0.0;
        
        mAlgorithm=algorithm;
        //random number from 0 to 30, inclusively
        if(algorithm < 0)mAlgorithm=0;
        if(algorithm>30)mAlgorithm=30;
...
{
    assert(t >= 0 && t <= 1.0);
    return sin(-13.0 * (t + 1.0) * M_PI_2) * pow(2.0, -10.0 * t) +
           1.0;
}
